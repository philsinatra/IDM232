...
^ Our client has a new project request. They want to create a web based game application that mimics Family Feud. They're going to use this app to reinforce training topics that students have already studied in a classroom setting. The client will provide (_click_) the topics and (_click_) the answers that are revealed for each topic.
...
^ So the UI design portion of this is set - we can go off and work on sketches, wireframes, style tiles and being building the front end in the browser. But what about the back end? Dealing with the data? The files are going to run on a web server, but we will not have PHP or MySQL available in the production environment.
...
^ I can tell you in this example, we definitely can still use PHP, and we may even use MySQL. Can anyone tell me how? Let's examine the data from the client. (_examples/week7/data.xlsx_)
...
^ Navigating to the page in the browser is simple, requires less work. Every time we want to regenerate the files, we just have to refresh the page. Simple, but not elegant. Okay for a dev environment, but we wouldn't want to keep the process this rudimentry beyond our own desktop.
...
^ We could build a form that when submitted, moves to the `gen.php` script, executes it and then returns us to our form page. It's not to complicated, but still not very eligant since there woudl be two page reloads to complete the process.
...
^ AJAX stands for Asynchronous JavaScript and XML. In a nutshell, it is the use of the XMLHttpRequest object to communicate with server-side scripts. It can send as well as receive information in a variety of formats, including JSON, XML, HTML, and even text files. AJAX’s most appealing characteristic is its "asynchronous" nature which means it can communcitare with the server, exchange data, and update the page all without having to refresh the browser.
...
^ Running the script from the command line would be simple, but much like some of the earlier options it would not be as elegant, and would require shell access which may not be possible outside of your local machine.
